defmodule OrcaHub.Artifacts.Render do
  @moduledoc """
  Builds the exact bytes served at `/artifacts/:id/raw` (and `/download`) —
  extracted from `OrcaHubWeb.ArtifactController` so non-web callers (e.g.
  `OrcaHub.MCP.Tools.Artifacts`'s local playwright screenshot shell-out) can
  render the same document without going through the HTTP layer.
  """
  ...
  def content_type(_html_or_markdown), do: "text/html"

  def body(%{kind: "svg", content: content}), do: content || ""

  def body(%{kind: "markdown", content: content}) do
    """
    <!doctype html>
    <html>
      <head>
  ...
  def body(%{kind: "html", content: content, data: data}) do
    inject_scripts(content || "", data || %{})
  end

  def body(%{content: content}), do: content || ""
