defmodule OrcaHub.MCP.Tools.Discord do
  ...
  defp confine(directory, path) do
    case PathConfinement.confine(directory, path, allowed_roots(directory)) do
      {:ok, resolved} ->
        {:ok, resolved}
      ...
      {:error, :outside_root} ->
        ...
        # Deliberately non-leaky: name the path as the CALLER supplied it,
        # say it's outside the session directory, and never echo the
        # resolved path (a real filesystem path, potentially outside the
        # sandbox entirely) back to a Discord channel.
        {:error, "File path #{inspect(path)} is outside the session directory."}
    end
  end

  @doc "See `OrcaHub.PathConfinement.realpath/1` — kept here for existing callers/tests."
  def realpath(path), do: PathConfinement.realpath(path)
