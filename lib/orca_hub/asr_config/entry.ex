defmodule OrcaHub.ASRConfig.Entry do
  ...
  exactly ONE kind here:
    ...
    * `"asr_provider"` — the active transcription settings. Exactly one row,
      ...
      Any key may be blank or absent, in which case that ONE field falls
      back to its `ASR_*` env var — see `OrcaHub.ASRConfig.resolve/0`.
      `enabled: false` disables the whole row, reverting every field to env
