defmodule OrcaHubWeb.IssueLive.Show do
  ...
  def handle_event("start_session", _params, socket) do
    ...
    params =
      if issue.project,
        do: %{directory: issue.project.directory, project_id: issue.project.id},
        else: %{}

    params =
      params |> Map.put(:issue_id, issue.id) |> Map.put(:runner_node, Atom.to_string(runner_node))
      ...
      {:error, _changeset} ->
        {:noreply,
         put_flash(
           socket,
           :error,
           "Could not create session. Make sure the issue has a project with a directory."
         )}
    end
  end
