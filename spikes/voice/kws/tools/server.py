#!/usr/bin/env python3
"""Static server for the SPIKE 3 kws harness + a tiny WAV/JSON upload receiver.

Derived from SPIKE 1's tools/server.py with three changes: the document root
is spikes/voice/ (the PARENT) so /vendor/ort/ and /capture-worklet.js resolve
to SPIKE 1's read-only copies while this spike's files stay under /kws/;
uploads land in kws/out/; and the default port is 8791 rather than 8777 so
both spikes' servers can run at the same time.
...
if __name__ == "__main__":
    ...
    print("SPIKE 3 kws harness: http://localhost:%d/  (coi=%s, root=%s)"
