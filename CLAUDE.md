...
- **Voice mode** (phases 1-2b, see `.context/voice-mode.md` for the pipeline, the OVS1 wire contract and the invariants): `OrcaHubWeb.VoiceChannel` (transport + effects), `OrcaHub.Voice.Session` (pure state machine), `OrcaHub.Voice.ASR` (GB10 sync lane) and `OrcaHub.Voice.Intent` (phonetic command matcher), configured by `OrcaHub.ASRConfig`; the browser half is the `Voice` LiveView hook (`assets/js/voice/`), hosted by `OrcaHubWeb.VoiceBarLive` — a STICKY nested LiveView in the app header, so every internal link must live-navigate (`<.link navigate>`/`push_navigate`) or the mic, `AudioContext` and channel die with the page. Assistant text streams live via `OrcaHub.Backend.Deltas` (one normalized `orca_delta` kind per backend -> unpersisted `session:<id>` broadcasts), feeding the in-progress bubble and the streaming-TTS accumulator (`assets/js/tts_stream.js`).
...
- Archiving a session (`archive_session`, default `extract_memories: true`) auto-dispatches `OrcaHub.MemoryExtraction` for orchestrators and root/human-driven sessions (child workers are skipped by default — override per-session with `memory_extract`, or force it on demand with the orchestrator-only `extract_memories` tool). A cheap background session (`sessions.kind: "memory_extraction"`, hidden from the sessions index/`search_sessions` by default — see the `kind` field/"Show background sessions" toggle) reviews new transcript since the last extraction and calls `remember`/`update_memory`, then posts a summary into the source session's feed and self-archives — `OrcaHub.SessionRunner` calls `MemoryExtraction.finalize_self/2` directly at its turn-end idle/error transition, no separate watcher process to lose on a restart (`OrcaHub.MemoryExtractionSweep` is the hub-only boot-time backstop for whatever still slips through). Pass `extract_memories: false` to archive without triggering it.
...
- User-initiated archiving (the MCP `archive_session` tool, and both LiveViews' archive/undo/bulk-archive actions) cascades: `OrcaHub.Cluster.cascade_archive_session/3` also archives every unarchived descendant in the session's whole spawn subtree, resolving each one's node (never re-assigning across nodes) and skipping — without pruning the rest of that branch — any descendant still `running`/`waiting`. The non-cascading archive paths (empty-session auto-archive, memory-extraction self-archive, `MemoryExtractionSweep`, `archive_on_complete` triggers) still call `Sessions.archive_session/2` directly and archive only themselves.
- A fresh orchestrator's system prompt lists other active sessions in its own directory (`SharedPrompts.active_sessions_prompt/3` — "active" means running/waiting at any age, or updated within the last 24h; not archived, not a background `kind`), orchestrators sorted first, with instructions to call `search_sessions` with no arguments for the live picture and check in about file ownership before spawning workers. pi carries this in `ORCA_IDENTITY` rather than its flags prompt, same reasoning as `open_issues_prompt`.
