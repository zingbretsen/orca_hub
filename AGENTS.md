...
- There are two deploy paths. A full rollout (images + OTP releases, via `~/homelab/scripts/deploy-orca-hub.sh`) is still the only path for dependency, supervision-tree, config and migration changes, anything `OrcaHub.Cluster.HotLoadGate` refuses, and the hot-deploy machinery itself. The other is a published **code generation**: beams stored in the DB (`code_generations`) that `OrcaHub.Cluster.CodePush` reconciles onto every node — durable across restarts, since the hub re-applies the stored generation to itself at boot and to each agent on `nodeup`. Ad-hoc/manual hot-loading outside a generation is still not durable. The per-node `OrcaHub.Cluster.CodeStamp` deliberately does NOT survive a restart: a restarted node really is back on image code until it is reconciled again (seconds later). See `.context/code-deploy.md`.
