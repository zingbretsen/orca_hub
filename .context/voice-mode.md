# Voice Mode (phase 1 shipped, phase 2 in progress)
...
panel; this file is the map and the invariants for what is BUILT (phase 1) —
see "Phase 2" at the bottom for what is landing now.
...
sequenceDiagram
    ...
    participant VAD as Silero v5<br>(vad-web)
    ...
    WL->>WL: getUserMedia -> 63-tap LPF + decimate to 16 kHz;<br>ring buffer keyed by ABSOLUTE 16 kHz index
...
- Client -> server: `speech_start`, `mic`, `send_now`, `cancel`, `draft_edit`,
  `retry_warmup` (phase 2b adds four, §8.2). Server -> client: `state` (the FULL
  snapshot, after every change), `segment_result` (one per segment, with the
  `action` taken), `sent`. Join errors:
  `not_found | voice_owned | node_unavailable | archived`. Joining IS arming —
  the warm-up ping fires at once (cold start up to 35 s).
...
- **The arming window opens LATE, so it is also skipped retroactively**: 600 ms
  ...
  at all (action `send`, detail "arming skipped: speech resumed"). Speech
  resuming inside the first 600 ms merges into the same segment, so no command
  is detected at all.
- **Sending is `:queue`, never `:interrupt`** (`TriggerExecutor` precedent): a
  spoken "send" must not cancel an in-flight turn. In phase 1 the SERVER
  delivers on arming expiry and the client never submits the form (that would
  double-send); phase 2b inverts exactly this — see §8.2's single send path.
- **The draft lives in the composer textarea, which the hook does not own.**
  ...
  typing: composer edits push `draft_edit` (300 ms debounce) and seed the server
  at join, and an empty `state.draft` NEVER empties a non-empty composer —
  clearing is explicit only (`"sent"`, a `cancel` segment_result,
  `clear-prompt`). The composer outlives the hook, so `destroyed()` must remove
  that listener. Phase 2b indirects the sink through
  `form[data-voice-composer-for]` (§8.2).
...
k3s **`orca-hub` pod** alone (agent-mode nodes 404 every page route; port-4001
LAN instances fail `getUserMedia`'s secure-origin rule). VERIFIED 2026-09-17
from inside the pod: `http://192.168.1.77:8000/healthz` -> 200 in ~5 ms, sync
lane ready, CUDA up, egress unrestricted. The URL is a config knob; two other
valid targets and one DNS trap (`gb10.lab.ingbretsenhome.com` is the debian
Traefik wildcard, NOT the GB10) are in spec §8 — read it before changing it.

## Phase 2 (in progress)

Phase 1 is all that is DEPLOYED (`d679c12`). Landing now, per
`voice_mode_spec.md` v0.5 — read the spec section, this file does not restate
the contracts (§10.5 is the contract -> phase -> section map):

- **§7.1-7.3 (C1-C3), phase 2** — unpersisted assistant text deltas on the
  existing `session:<id>` topic, forwarded as one `assistant-stream`
  push_event, feeding both a live bubble and a sentence accumulator that speaks
  prose mid-turn behind a "Speak while streaming" toggle.
- **§8.2 (C4), phase 2b** — `OrcaHubWeb.VoiceBarLive`, a sticky app-header bar
...
adjudicator (5); `:stop`/`:pause` are recognized and stripped but take no
action. **Phase 1's EXIT CRITERIA still await a human** — both parts of
`spikes/voice/ACOUSTIC_TEST.md`: Part A (real-hardware AEC suppression, which
gates phases 3-4) and Part B (the real-voice matcher check, >= 95% TP / 0 FP at
0.85 — the 96.8%/0.0% above is one synthetic voice, an upper bound until it
runs). Neither gates phase 2.
