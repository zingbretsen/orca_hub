...
const TTSMethods = {
  ...
  ttsHeaders() {
    ...
    if (this.ttsApiToken)
      headers["authorization"] = `Bearer ${this.ttsApiToken}`;
    return headers;
  },
  ...
  ttsCleanText(text) {
    ...
    text = text.replace(/(?:\/[\w.-]+)+\/([\w.-]+)/g, (match, filename) => {
      return filename.replace(/_/g, " ").replace(/\.(\w+)$/, " dot $1");
    });

    // Standalone filenames (word.ext): "show.ex" -> "show dot ex"
    text = text.replace(
      /\b(\w[\w-]*)\.(ex|exs|js|ts|css|html|json|md|yml|yaml|toml|txt|rb|py|go|rs|sh|heex|eex|leex)\b/g,
  ...
  ttsFetchAudio(index) {
    if (this.audioCache[index] != null)
      return Promise.resolve(this.audioCache[index]);
  ...
  ttsUpdateUI(id) {
    ...
    if (this.chunks.length === 0) {
      ...
      if (counter)
        counter.textContent = `${this.currentIndex + 1}/${this.chunks.length}`;
      if (playBtn)
        playBtn.innerHTML = this.playing ? TTS_ICON_PAUSE : TTS_ICON_PLAY;
    }
  },
...
let Hooks = {
    ...
    _setupResize() {
      ...
      const onMouseMove = (e) => {
        ...
        const leftWidth = Math.max(
          minPx,
          Math.min(x, rect.width - minPx - gap),
        );
  ...
  ScrollToLine: {
    ...
    scrollToTarget() {
      ...
      const targetId = isBlock
        ? `${prefix}file-block-${line}`
    ...
    selectItem(item) {
      ...
      if (this.trigger === "/") {
        ...
        this.el.selectionStart = this.el.selectionEnd =
          beforeTrigger.length + insertion.length;
      }
  ...
  ScrollToBottom: {
    ...
    updated() {
      ...
      if (this.following) {
        ...
        const anchor =
          this._anchorId && document.getElementById(this._anchorId);

        if (anchor) {
          this.el.scrollTop +=
            anchor.getBoundingClientRect().top - this._anchorOffset;
        } else {
  ...
  ArtifactData: {
    mounted() {
      ...
      this._onMessage = (e) => {
        ...
        if (e.data?.type === "orca:send") {
          ...
          this.pushEvent("artifact_state", {
            artifact_id: this.el.dataset.artifactId,
            patch: e.data.patch,
          });
        }
      };
    ...
    _handleOrcaSend(payload) {
      ...
      if (now - this._lastSendAt < 500) {
        console.warn(
          "orca.send: rate limit exceeded (max 1 per 500ms), dropped",
        );
        return;
      }
      this._lastSendAt = now;

      this.pushEvent("artifact_send", {
        artifact_id: this.el.dataset.artifactId,
        payload,
      });
    },
  ...
  FileTree: {
    _getVisibleItems() {
      return Array.from(
        this.el.querySelectorAll(
          "li > button[phx-click='select_file'], li > details > summary",
        ),
      );
    },
    _focusItem(item) {
      ...
      this.el
        .querySelectorAll(".file-tree-focused")
        .forEach((el) => el.classList.remove("file-tree-focused"));
    ...
    beforeUpdate() {
      this._detailsState = Array.from(this.el.querySelectorAll("details")).map(
        (d) => d.open,
      );
