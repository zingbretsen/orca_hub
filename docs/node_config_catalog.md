...
| 3 | `extensions/` | dir | `.ts` (or `.js`) files, one pi `ExtensionAPI` module each | Custom tools/hooks loaded at session start. Pi memory is no longer sourced from an on-disk extension file here — it now comes from the hub's centralized memory service, injected at spawn time via the `ORCA_MEMORY` env var (built in `lib/orca_hub/backend/pi.ex`) and consumed by `priv/pi/orca-memory.ts`, which OrcaHub writes into the session's own runtime, not into this global directory. The old per-node `extensions/claude-memories.ts` bridge (formerly checked in at `scripts/pi-extensions/claude-memories.ts`, now deleted) is obsolete; any leftover `~/.pi/agent/extensions/claude-memories.ts` on a node should be removed | N/A — these are code, not data; a sensible "create" affordance is a boilerplate `ExtensionAPI` module, not an empty file | Executable code — treat edits as higher-risk than data files; changes apply on next pi session start |
